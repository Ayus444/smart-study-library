...
// POST /api/payments
// Finds existing Pending/Overdue record for same student+month and updates it to Paid
// No duplicate records ever created
router.post('/', protect, async (req, res) => {
  try {
    ...
    // Update existing pending/overdue record for this month — no duplicates!
    const payment = await Payment.findOneAndUpdate(
      { studentId, month, status: { $in: ['Pending', 'Overdue'] } },
      {
        $set: {
    ...
    // Update student fee status and next due date
